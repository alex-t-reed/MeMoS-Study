...
expName = 'memos_baseline_task'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/alexreed/Desktop/Coding/MeMoS-Study/memos-study-baseline/MeMoS_Baseline_lastrun.py',
...
faceset1 = ["resources/faceset1/faceset1_" + str(num) + ".jpg" for num in range(1, num_items + 1)]
faceset2 = ["resources/faceset2/faceset2_" + str(num) + ".jpg" for num in range(1, num_items + 1)]
faceset3 = ["resources/faceset3/faceset3_" + str(num) + ".jpg" for num in range(1, num_items + 1)]
faceset4 = ["resources/faceset4/faceset4_" + str(num) + ".jpg" for num in range(1, num_items + 1)]
faceset5 = ["resources/faceset5/faceset5_" + str(num) + ".jpg" for num in range(1, num_items + 1)]
faceset6 = ["resources/faceset6/faceset6_" + str(num) + ".jpg" for num in range(1, num_items + 1)]
...
# Initialize components for Routine "FreeResponse_1"
FreeResponse_1Clock = core.Clock()
Question_1 = visual.TextStim(win=win, name='Question_1',
    text='Free Response Question 1 \n\nDid you use any strategies on this task? What are they?',
    font='Open Sans',
    pos=(0, 0.3), height=0.04, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=0.0);
strategies = visual.TextBox2(
...
text_6 = visual.TextStim(win=win, name='text_6',
    text='Please wait for the experimenter to save your score.',
    font='Open Sans',
    pos=(0, -.35), height=0.04, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
thankyoumsg = visual.TextStim(win=win, name='thankyoumsg',
    text='You have completed part one of the experiment. \n\nThank you! \n\nPress the ‘e’ key to exit.  \n',
    font='Open Sans',
...
while continueRoutine:
    ...
    # *key_resp_16* updates
    waitOnFlip = False
    if key_resp_16.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
        # keep track of start time/frame for later
...
while continueRoutine:
    ...
    # *key_resp_17* updates
    waitOnFlip = False
    if key_resp_17.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
        # keep track of start time/frame for later
...
while continueRoutine:
    ...
    # *key_resp_14* updates
    waitOnFlip = False
    if key_resp_14.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
        # keep track of start time/frame for later
...
# ------Prepare to start Routine "FreeResponse_1"-------
continueRoutine = True
# update component parameters for each repeat
strategies.reset()
...
gotValidClick = False  # until a click is received
# keep track of which components have finished
FreeResponse_1Components = [Question_1, strategies, mouse_1, Clickable_1]
for thisComponent in FreeResponse_1Components:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
FreeResponse_1Clock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "FreeResponse_1"-------
while continueRoutine:
    # get current time
    t = FreeResponse_1Clock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=FreeResponse_1Clock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    ...
    # *strategies* updates
    if strategies.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        strategies.frameNStart = frameN  # exact frame index
        strategies.tStart = t  # local t and not account for scr refresh
        strategies.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(strategies, 'tStartRefresh')  # time at next scr refresh
        strategies.setAutoDraw(True)
    # *mouse_1* updates
...
# -------Ending Routine "FreeResponse_1"-------
for thisComponent in FreeResponse_1Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('Question_1.started', Question_1.tStartRefresh)
thisExp.addData('Question_1.stopped', Question_1.tStopRefresh)
thisExp.addData('strategies.text',strategies.text)
thisExp.addData('strategies.started', strategies.tStartRefresh)
thisExp.addData('strategies.stopped', strategies.tStopRefresh)
# store data for thisExp (ExperimentHandler)
thisExp.nextEntry()
...
# the Routine "FreeResponse_1" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
while continueRoutine:
    ...
    if key_resp_20.status == STARTED and not waitOnFlip:
        theseKeys = key_resp_20.getKeys(keyList=['m'], waitRelease=False)
